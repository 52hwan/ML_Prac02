...
""" Comparing with ML_Prac01's mnist_deep.py is useful"""
...
def cnn_model_fn(features, labels, mode):
    ...
    # Here, -1 for batch size specifies that this dimension should be dynamical.
    ...
    conv2 = tf.layers.conv2d(inputs=pool1, filters=64,
                             kernel_size=[5,5], padding="same",
                             activation=tf.nn.relu)
    # Note: The input has 32 channels. channel of the output = 32 * 64?
    # No. The filter is actually [5,5,32]. A single feature map is 1-channel.
    ...
    # The default activation function: linear

    predictions = {
        "classes": tf.argmax(input=logits, axis=1),
        # axis=1, since logits=[batch_size, 10]
